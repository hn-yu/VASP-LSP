"""Focused fixture/golden test for the vasp.incar.invalid_value rule (#52).
...
- ``tests/fixtures/rules/invalid_incar_value/INCAR``      -> invalid fixture
- ``tests/fixtures/rules/invalid_incar_value/valid_INCAR`` -> valid non-triggering fixture
- ``tests/fixtures/rules/invalid_incar_value.json``       -> golden assertions
"""
...
FIXTURE_DIR = Path(__file__).parent / "fixtures" / "rules" / "invalid_incar_value"
GOLDEN_PATH = Path(__file__).parent / "fixtures" / "rules" / "invalid_incar_value.json"
...
def test_invalid_fixture_emits_exactly_this_rule() -> None:
    """The invalid fixture must produce exactly one vasp.incar.invalid_value diagnostic."""
    golden = _load_golden()
...
def test_valid_fixture_does_not_trigger_rule() -> None:
    """A valid INCAR must not emit vasp.incar.invalid_value."""
