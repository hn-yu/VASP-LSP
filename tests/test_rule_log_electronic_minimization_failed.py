"""Focused fixture/golden test for the vasp.log.electronic_minimization_failed rule (#59)."""
...
GOLDEN_PATH = (
    Path(__file__).parent / "fixtures" / "rules" / "log_electronic_minimization_failed.json"
)
...
def test_invalid_fixture_emits_exactly_this_rule() -> None:
    """The electronic-minimization log must produce at least one
    vasp.log.electronic_minimization_failed diagnostic.
    """
    golden = _load_golden()
    items = _check_fixture(LOG_FIXTURE_DIR / "electronic_minimization_failed.out")
...
def test_valid_fixture_does_not_trigger_rule() -> None:
    ...
    items = _check_fixture(LOG_FIXTURE_DIR / "electronic_minimization_failed_valid.out")
    matching = [item for item in items if item["code"] == RULE_ID]
