"""Focused fixture/golden test for the vasp.smearing.ismear_sigma_mismatch rule (#54).
...
- ``tests/fixtures/rules/ismear_sigma_mismatch/INCAR``      -> invalid fixture
- ``tests/fixtures/rules/ismear_sigma_mismatch/valid_INCAR`` -> valid non-triggering fixture
- ``tests/fixtures/rules/ismear_sigma_mismatch.json``       -> golden assertions
"""
...
FIXTURE_DIR = Path(__file__).parent / "fixtures" / "rules" / "ismear_sigma_mismatch"
GOLDEN_PATH = Path(__file__).parent / "fixtures" / "rules" / "ismear_sigma_mismatch.json"
...
def test_invalid_fixture_emits_exactly_this_rule() -> None:
    """The invalid fixture must produce exactly one vasp.smearing.ismear_sigma_mismatch diagnostic."""
    golden = _load_golden()
...
def test_valid_fixture_does_not_trigger_rule() -> None:
    """A valid INCAR (ISMEAR >= 0 paired with SIGMA) must not emit the rule."""
