"""Focused fixture/golden test for the vasp.spin.missing_magmom rule (#53).
...
- ``tests/fixtures/rules/missing_magmom/INCAR``      -> invalid fixture
- ``tests/fixtures/rules/missing_magmom/valid_INCAR`` -> valid non-triggering fixture
- ``tests/fixtures/rules/missing_magmom.json``       -> golden assertions
"""
...
FIXTURE_DIR = Path(__file__).parent / "fixtures" / "rules" / "missing_magmom"
GOLDEN_PATH = Path(__file__).parent / "fixtures" / "rules" / "missing_magmom.json"
...
def test_invalid_fixture_emits_exactly_this_rule() -> None:
    """The invalid fixture must produce exactly one vasp.spin.missing_magmom diagnostic."""
    golden = _load_golden()
...
def test_valid_fixture_does_not_trigger_rule() -> None:
    """A valid spin-polarized INCAR (ISPIN=2 with MAGMOM) must not emit the rule."""
