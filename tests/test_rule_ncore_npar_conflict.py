"""Focused fixture/golden test for the vasp.parallel.ncore_npar_conflict rule (#56).
...
- ``tests/fixtures/rules/ncore_npar_conflict/INCAR``       -> invalid fixture (both flags)
- ``tests/fixtures/rules/ncore_npar_conflict/valid_INCAR`` -> valid non-triggering fixture
- ``tests/fixtures/rules/ncore_npar_conflict.json``        -> golden assertions

NCORE and NPAR are mutually-exclusive parallelism flags
(https://www.vasp.at/wiki/index.php/NCORE and
https://www.vasp.at/wiki/index.php/NPAR); VASP derives one from the other,
so declaring both is contradictory.
"""
...
FIXTURE_DIR = Path(__file__).parent / "fixtures" / "rules" / "ncore_npar_conflict"
GOLDEN_PATH = Path(__file__).parent / "fixtures" / "rules" / "ncore_npar_conflict.json"
...
def test_invalid_fixture_emits_exactly_this_rule() -> None:
    """Both NCORE and NPAR set must produce exactly one rule diagnostic."""
    golden = _load_golden()
...
def test_valid_fixture_does_not_trigger_rule() -> None:
    """An INCAR with only NCORE (no NPAR) must not emit the rule."""
    items = _check_fixture(FIXTURE_DIR / "valid_INCAR")
    matching = [item for item in items if item["code"] == RULE_ID]
    assert matching == [], f"valid fixture unexpectedly emitted {RULE_ID}: {matching}"


def test_only_npar_does_not_trigger_rule() -> None:
    """An INCAR with only NPAR (no NCORE) must not emit the rule."""
    items = _check_text("ENCUT = 520\nNPAR = 4\n")
    matching = [item for item in items if item["code"] == RULE_ID]
    assert matching == [], f"only-NPAR fixture unexpectedly emitted {RULE_ID}: {matching}"


def test_neither_flag_does_not_trigger_rule() -> None:
    """An INCAR with neither NCORE nor NPAR must not emit the rule."""
    items = _check_text("ENCUT = 520\nISMEAR = 0\nSIGMA = 0.05\n")
    matching = [item for item in items if item["code"] == RULE_ID]
    assert matching == [], f"neither-flag fixture unexpectedly emitted {RULE_ID}: {matching}"


def test_message_still_mentions_both_flags() -> None:
    """The message must still name both NPAR and NCORE so quickfixes match."""
    items = _check_fixture(FIXTURE_DIR / "INCAR")
    matching = [item for item in items if item["code"] == RULE_ID]
    assert matching, "rule did not fire on the invalid fixture"
    message = matching[0]["message"]
    assert "NPAR" in message and "NCORE" in message, message
